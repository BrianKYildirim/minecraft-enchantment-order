...
from .utils import weight, xp_from_levels, check_item_can_have, is_compatible, MAX_MERGE_LEVELS
...
class EnchantedItem:
    ...
    def __post_init__(self):
        object.__setattr__(self,
                           "value",
                           sum(level * weight(ns) for ns, level in self.enchants.items())
                           )
    ...
    def from_state(item_type: str, enchants: Dict[str, int] | None = None, *, anvil_uses: int = 0):
        ...
        for ns, lv in enchants.items():
            if not check_item_can_have(ns, item_type):
                raise ValueError(f"{item_type} cannot receive '{ns}'")
            if lv < 1 or lv > ENCHANTMENTS[ns]["levelMax"]:
                raise ValueError(f"Invalid level {lv} for '{ns}'")
        return EnchantedItem(item_type, dict(enchants), anvil_uses)

    @staticmethod
    def book(ns: str, level: int):
        return EnchantedItem("book", {ns: level})

    def prior_penalty(self) -> int:
        return (1 << self.anvil_uses) - 1

    def merge_cost_levels(self, other: "EnchantedItem", rename=False, repair=False) -> int:
    ...
    def merge(self, other: "EnchantedItem", *, rename=False, repair=False):
        ...
        if cost_lv > MAX_MERGE_LEVELS:
            raise MergeTooExpensive(f"Cost {cost_lv} > 39")
        ...
        for ns, lv in other.enchants.items():
            curr = new_enchants.get(ns, 0)
            if curr != lv:
                new_enchants[ns] = max(curr, lv)
            else:
                new_enchants[ns] = min(curr + 1, ENCHANTMENTS[ns]["levelMax"])

        ok, conf = is_compatible(new_enchants)
        if not ok:
            raise IncompatibleSelected(conf)

        new_type = self.item_type if self.item_type != "book" else other.item_type
        return (EnchantedItem(new_type, new_enchants, anvil_uses=new_uses),
                cost_lv, cost_xp)

    def pretty(self) -> str:
        if self.item_type == "book":
            txt = ", ".join(f"{ns} {lv}" for ns, lv in self.enchants.items())
            return f"Book ({txt})"
        if self.enchants:
            parts = []
            for ns, lv in sorted(self.enchants.items()):
                lvl_txt = f" {lv}" if ENCHANTMENTS[ns]["levelMax"] > 1 else ""
                parts.append(f"{ns.replace('_', ' ').title()}{lvl_txt}")
            core = f"{self.item_type.title()} ({', '.join(parts)})"
        else:
            core = self.item_type.title()
...
class Step:
    ...
    def __str__(self):
        ...
        if l.item_type == "book" and r.item_type != "book":
            l, r = r, l
        return (f"Combine {l.pretty()} + {r.pretty()} → "
                f"{self.cost_levels} Levels ({self.cost_xp}xp), "
                f"penalty={self.result_prior}")
